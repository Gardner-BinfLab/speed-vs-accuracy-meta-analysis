...
for(id in keys){       
       len<-length(dr$normAccuracyRank[ dr$testId == id])       
       spear<-cor.test(dr$normAccuracyRank[ dr$testId == id], dr$normSpeedRank[ dr$testId == id], method = "spearman", alternative = "less")
